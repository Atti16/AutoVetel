...
function getItems(req,res) {
    ...
    var data = fs.readFileSync('../Frontend/autoInfo.json');
    res.send(data);
}
...
function addNewItem(req,res) {
    let auto = req.body;
    if (! fs.existsSync('../Frontend/autoInfo.json')) {
        res.status(404);
    ...
    auto.id = lastId.toString();
    var newJson = '{ ';
    for (var i = 1; i <= Object.keys(data2).length; i++) {
        newJson +=  '"'+ i +'": ' + JSON.stringify(data2[i], null, 2) + ', ';
    }
    var d = '"'+ lastId +'": '+ JSON.stringify(auto, null, 2);
    newJson += d;
    newJson += '}';
    fs.writeFileSync('../Frontend/autoInfo.json',newJson);
    console.log('Item added');
    reply = { status: "added", auto: auto }
    res.send(reply);
}
